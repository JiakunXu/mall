...
public class ResponseHandler {
	...
	// modify by xujiakun 2014-9-4
	private SortedMap<String, String> postData;
	...
	public ResponseHandler(HttpServletRequest request,
		...
		// modify by xujiakun 2014-9-4
		postData = new TreeMap<String, String>();
		InputStream in = null;
		try {
			...
			for (Element e : elementList) {
				postData.put(e.getName().toLowerCase(), e.getText());
			}
		} catch (IOException e) {
			logger.error(e);
	...
	/**
	 * 判断微信签名(modify by xujiakun 2014-9-4).
	 * 
	 * @return
	 */
	public boolean isWXsign() {
		postData.put("appkey", WxapConfig.APP_KEY);

		StringBuilder sb = new StringBuilder();

		for (Map.Entry<String, String> m : postData.entrySet()) {
			String k = m.getKey();
			if (!"signmethod".equals(k) && !"appsignature".equals(k)) {
				if (sb.length() != 0) {
					sb.append("&");
				}
				sb.append(k.toLowerCase() + "=" + m.getValue());
			}
		}
		...
		return postData.get("appsignature").equals(sign);
	}
	...
	public SortedMap<String, String> getPostData() {
		return postData;
	}

	public void setPostData(SortedMap<String, String> postData) {
